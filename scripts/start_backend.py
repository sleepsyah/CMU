...
def warmup() -> None:
    print("Warming backend models. First run may download and load model files...")
    payload = request_json("/warmup", timeout=180)
    if not payload:
        raise RuntimeError(f"Warmup failed. Check {LOG_FILE}.")
    print(json.dumps(payload, indent=2))
    if not payload.get("ready"):
        print("Backend is reachable, but one or more optional/required models did not load.")
